#' Expanding the bigdata package to use the LASSO Logic Engine
#' 
#' Add LASSO Logic Engine functionality to the biglasso package, which extended
#' lasso and elastic-net linear, logistic and cox regression models for
...
#' 
#' \tabular{ll}{ Package: \tab largeLLE\cr Type: \tab Package\cr Version: \tab
#' 0.0.0\cr Date: \tab 2022-12-04\cr License: \tab GPL-3\cr}
#' 
...
#' features of the original \code{biglasso} are summarized as below:
#'  \enumerate{ \item it utilizes memory-mapped files to store the massive data
#' on the disk, only loading data into memory when necessary during model
#' fitting. Consequently, it's able to seamlessly data-larger-than-RAM cases.
#' \item it is built upon pathwise coordinate descent algorithm with warm start,
#' active set cycling, and feature screening strategies, which has been proven
#' to be one of fastest lasso solvers. \item in incorporates our newly developed
#' hybrid and adaptive screening that outperform state-of-the-art screening
#' rules such as the sequential strong rule (SSR) and the sequential EDPP rule
#' (SEDPP) with additional 1.5x to 4x speedup. \item the implementation is
#' designed to be as memory-efficient as possible by eliminating extra copies of
#' the data created by other R packages, making it at least 2x more memory-
#' efficient than \code{glmnet}. \item the underlying computation is implemented
#' in C++, and parallel computing with OpenMP is also supported. }
#' 
...
#' }
#' 
#' @author Jason Orender, Yaohui Zeng, Chuyi Wang and Patrick Breheny
#' 
#' Maintainer: Jason Orender<joren001@@odu.edu>
#' @references \itemize{ \item Orender, J., Zubair, M., Sun, J. (2022).  LASSO
#'  Logic Engine: harnessing the logic parsing capabilities of the LASSO
#'  algorithm for longitudinal feature learning. \url{https://github.com/jworender/lle}.
#' \item  Zeng, Y., and Breheny, P. (2017). The biglasso Package: A Memory- and
#'  Computation-Efficient Solver for Lasso Model Fitting with Big Data in R.
#' \url{https://arxiv.org/abs/1701.05936}.  \item Tibshirani, R., Bien, J.,
#' Friedman, J., Hastie, T., Simon, N., Taylor, J., and Tibshirani, R. J.
#' (2012). Strong rules for discarding predictors in lasso-type problems.
#' \emph{Journal of the Royal Statistical Society: Series B (Statistical
#' Methodology)}, \strong{74}(2), 245-266.  \item Wang, J., Zhou, J., Wonka, P.,
#' and Ye, J. (2013). Lasso screening rules via dual polytope projection.
#' \emph{In Advances in Neural Information Processing Systems}, pp. 1070-1078.
#' \item Xiang, Z. J., and Ramadge, P. J. (2012). Fast lasso screening tests
#' based on correlations. \emph{In Acoustics, Speech and Signal Processing
#' (ICASSP), 2012 IEEE International Conference on} (pp. 2137-2140). IEEE.
#' \item Wang, J., Zhou, J., Liu, J., Wonka, P., and Ye, J. (2014). A safe
#' screening rule for sparse logistic regression. \emph{In Advances in Neural
...
#' }
#' 
#' @useDynLib largeLLE, .registration = TRUE
