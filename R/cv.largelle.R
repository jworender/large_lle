#' Cross-validation for largeLLE (functionally unchanged from biglasso)
#'
...
#'
#' The function calls \code{largeLLE} \code{nfolds} times, each time leaving
...
#' \code{\link{largeLLE}}, which is the number of OpenMP threads, but only for
#' the first call of \code{\link{largeLLE}} that is  run on the entire data. The
#' individual calls of \code{\link{largeLLE}} for the CV folds are run without
#' the \code{ncores} argument.)
#' @param ... Additional arguments to \code{largeLLE}.
...
#' observations are randomly assigned by \code{cv.largelle}.
#' @param trace If set to TRUE, cv.largelle will inform the user of its
...
#' @return An object with S3 class \code{"cv.largelle"} which inherits from
...
#' \item{fit}{The fitted \code{largeLLE} object for the whole data.}
...
#' @author Jason Orender, Yaohui Zeng and Patrick Breheny
#'
#' Maintainer: Jason Orender <joren001@@odu.edu>
#' @seealso \code{\link{largeLLE}}, \code{\link{plot.cv.largelle}},
#' \code{\link{summary.cv.largelle}}, \code{\link{setupX}}
#' @examples
#' \dontrun{
#' ## cv.largelle
#' data(colon)
...
#'
#' ## logistic regression
#' cvfit <- cv.largelle(X.bm, y, family = 'binomial', seed = 1234, ncores = 2)
#' par(mfrow = c(2, 2))
#' plot(cvfit, type = 'all')
#' summary(cvfit)
#' }
...
#' @export cv.largelle
#'
cv.largelle <- function(X, y, row.idx = 1:nrow(X),
  ...
  fit <- largeLLE(X = X, y = y, row.idx = row.idx, family = family, ncores = ncores, ...)
  ...
  val <- list(cve=cve, cvse=cvse, lambda=lambda, fit=fit, min=min, lambda.min=lambda[min], lambda.1se = lambda.1se,
              null.dev=mean(loss.largelle(y, rep(mean(y), n),
  ...
  structure(val, class=c("cv.largelle", "cv.ncvreg"))
}
...
cvf <- function(i, XX, y, eval.metric, cv.ind, cv.args, grouped, parallel= FALSE) {
  ...
  fit.i <- do.call("largeLLE", cv.args)

  y2 <- y[cv.ind==i]
  yhat <- matrix(predict(fit.i, XX, row.idx = idx.test, type="response"), length(y2))

  loss <- loss.largelle(y2, yhat, fit.i$family, eval.metric = eval.metric, grouped = grouped)
