...
test_that("Test against OLS:", {
  ...
  fit.ssr <- largeLLE(X.bm, y, screen = 'SSR', eps = eps, lambda = 0)
  fit.hybrid <- largeLLE(X.bm, y, screen = 'Hybrid', eps = eps, lambda = 0)
  fit.adaptive <- largeLLE(X.bm, y, screen = 'Adaptive', eps = eps, lambda = 0)
...
X.bm <- as.big.matrix(X)
fit.ssr <- largeLLE(X.bm, y, screen = 'SSR', eps = eps)
fit.hybrid <- largeLLE(X.bm, y, screen = 'Hybrid', eps = eps)
fit.adaptive <- largeLLE(X.bm, y, screen = 'Adaptive', eps = eps)


cvfit.ssr <- cv.largelle(X.bm, y, screen = 'SSR', eps = eps,
                         ncores = 1, cv.ind = fold)
cvfit.hybrid <- cv.largelle(X.bm, y, screen = 'Hybrid', eps = eps,
                              ncores = 1, cv.ind = fold)
cvfit.adaptive <- cv.largelle(X.bm, y, screen = 'Adaptive', eps = eps,
                              ncores = 1, cv.ind = fold)

## parallel computing
# fit.edpp.no.active2 <- largeLLE(X.bm, y, screen = 'SEDPP-No-Active', eps = eps, ncores = 2)
fit.ssr2 <- largeLLE(X.bm, y, screen = 'SSR', eps = eps, ncores = 2)
fit.hybrid2 <- largeLLE(X.bm, y, screen = 'Hybrid', eps = eps, ncores = 2)
fit.adaptive2 <- largeLLE(X.bm, y, screen = 'Adaptive', eps = eps, ncores = 2)
...
# Note: largeLLE has diverged from ncvreg in its approach to CV (#45)
...
X.bm <- as.big.matrix(X)
fit.ssr <- largeLLE(X.bm, y, penalty = 'enet', screen = 'SSR', eps = eps, alpha = alpha)
fit.ssr.edpp <- largeLLE(X.bm, y, penalty = 'enet', screen = 'Hybrid', eps = eps, alpha = alpha)

cvfit.ncv <- cv.ncvreg(X, y, penalty = 'lasso', eps = sqrt(eps), alpha = alpha,
                       lambda.min = lambda.min, fold = fold)
cvfit.ssr <- cv.largelle(X.bm, y, screen = 'SSR', penalty = 'enet', eps = eps, alpha = alpha,
                         ncores = 1, cv.ind = fold)
cvfit.ssr.edpp <- cv.largelle(X.bm, y, penalty = 'enet', screen = 'Hybrid', eps = eps, alpha = alpha,
