---
title: "largeLLE"
author: "Jason Orender (forked from biglasso by Yaohui Zeng, Chuyi Wang, Patrick Breheny)"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{largeLLE}
...
cvfit <- tryCatch(
         {
                cv.largelle(X.bm, y, seed = 1234, nfolds = 10, ncores = 4)
         },
         error = function(cond) {
                cv.largelle(X.bm, y, seed = 1234, nfolds = 10, ncores = 2)
         }
)
```
...
y <- Heart$y
X.bm <- as.big.matrix(X)
fit <- largeLLE(X.bm, y, family = "binomial")
plot(fit)
```
...
```{r}
system.time({fit <- largeLLE(X.bm, y)})
```
...
* largeLLE on GitHub: https://github.com/jworender/large_lle
